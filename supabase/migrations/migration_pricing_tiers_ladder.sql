-- Migration: ladder payouts for pricing_tiers
-- Implements milestone/step payouts as metrics grow ("лесенка").
    ...
    IF v_task.pricing_tiers IS NOT NULL
       ...
       AND jsonb_array_length(v_task.pricing_tiers) > 0 THEN
        ...
        FOR v_pricing_tier IN
            SELECT * FROM jsonb_array_elements(v_task.pricing_tiers)
        LOOP
            ...
            IF v_metric_value >= v_min THEN
                IF NOT EXISTS (
                    ...
                    -- Pay only if price > 0 (transactions.amount has > 0 constraint)
                    IF v_price > 0 THEN
                        ...
                        IF v_client_balance >= v_price THEN
                            UPDATE users
                            ...
                            INSERT INTO transactions (
                                ...
                                'task_payment',
                                'completed',
                                'Оплата по лесенке: ' || v_metric_name || ' >= ' || v_min::TEXT
                            );

                            v_new_payment := v_new_payment + v_price;
                            v_determined_price := v_determined_price + v_price;
                        ELSE
                            RAISE WARNING 'Insufficient client balance for tier payout: task %, client %, need %, have %', v_task.id, v_task.client_id, v_price, v_client_balance;
                            CONTINUE;
                        END IF;
                    END IF;

                    -- Mark tier as paid (even if price=0)
